...
// virtual
void RsFontWin32GL::Render( const MtVector2& v2Position,
							     ...
							     BtU32 sortOrder )
{
	...
	for( BtU32 nCharacterIndex=0; nCharacterIndex<length; nCharacterIndex++ )
	{
		...
		BtFloat fX1 = fX0 + ( v2Dimension.x * scale );
		BtFloat fY0 = v2Position.y + ( fontChar.m_nYOffset * scale );
		BtFloat fY1 = fY0 + ( v2Dimension.y * scale);
...
void RsFontWin32GL::Render( RsFontRenderable *pRenderable )
{
	...
	if(pRenderable->m_pTrueTypeFont )
	{
		pRenderable->m_pTrueTypeFont->Render( pRenderable->m_text, pRenderable->m_colour );
		
		pTexture = (RsTextureWinGL*)pRenderable->m_pTexture;
	}
