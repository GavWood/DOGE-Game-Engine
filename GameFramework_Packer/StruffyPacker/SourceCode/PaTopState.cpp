...
BtBool PaTopState::ReadConfig()
{
...
#else
    f = fopen( "/Users/localadmin/Documents/GitHub/Game-Framework/GameFramework_Packer/config.txt", "rb" );
#endif
    
	if( f == NULL )
	{
		BtStrCopy( m_szWorkingPath, 256, "." );
		ErrorLog::Printf( "Can't open config file. \n" );
		exit(0);
		return BtFalse;
	}
	...
	BtU32 nCurrentProject = 0;
	fscanf( f, "CurrentProject=%d\n", &nCurrentProject );
	for( BtU32 i=0; i<nCurrentProject + 1; i++ )
	{
		// Read in the working path
		fgets( m_szWorkingPath, 256, f );
		
		// Find a comment
		BtChar* pComment = strstr( m_szWorkingPath, "#" );
		...
		// Trim the spaces from the filename
		BtStrTrimSpaces( m_szWorkingPath, 256 );

		// Trim the newlines from the filename
		BtStrTrimNewLines( m_szWorkingPath, 256 );
