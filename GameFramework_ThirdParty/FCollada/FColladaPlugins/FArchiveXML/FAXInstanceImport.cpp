...
bool FArchiveXML::LinkControllerInstance(FCDControllerInstance* controllerInstance)
{
	...
	//int getJointCount = controllerInstance->GetJointCount();

	while (controllerInstance->GetJointCount() != 0)
	{
		controllerInstance->RemoveJoint(controllerInstance->GetJointCount() - 1);
	}

	for (size_t i = 0; i < jointCount; ++i)
	{
		const fm::string& jid = skin->GetJoint(i)->GetId();
		//const char *name = (const char*)jid;
		...
		if ( data.jointAreSids)
		{
			...
			for (size_t i = 0; i < numRoots; i++)
			{
				...
				if (boneNode != NULL)
				{
					const char *boneID = (const char*) boneNode->GetDaeId();
					(void)boneID;
					break;
				}
			}
