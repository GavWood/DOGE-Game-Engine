...
namespace FCDSceneNodeTools
{
	...
	void GenerateSampledAnimation( float maxAnimationDuration, float frameRate, FCDSceneNode* node)
	{
		sampleKeys.clear();
		sampleValues.clear();
		...
		int numFrames = (int)( maxAnimationDuration * frameRate );

		// Pre-allocate the value array;
		sampleValues.reserve(numFrames);

		float maxTime = sampleKeys[sampleKeyCount - 1];
