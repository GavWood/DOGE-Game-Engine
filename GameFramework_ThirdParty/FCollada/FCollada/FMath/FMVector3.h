...
public:
	...
	inline void Set(float nx, float ny, float nz) { this->x = nx; this->y = ny, this->z = nz; }
	...
	{
		float fx = (float)fabsf(x);
		float fy = (float)fabsf(y);
		float fz = (float)fabsf(z);
